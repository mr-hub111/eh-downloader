 ...
 * @param {(log: string) => T} logCallback - A function that will be called with a string parameter.
...
const getPageDetails = async (url = '', logCallback) => {
    ...
    if (logCallback) {
        logCallback(`${url} : contains ${pageDetails.length} pages`);
    }
...
const getPageGelleryDetails = async (url = '', logCallback) => {
    ...
    if (logCallback) {
        logCallback(`${url} : contains ${pageGalleryDetails.length} images`);
    }
 ...
 * @param {(log: string) => T} logCallback - A function that will be called with the log message.
...
const getImageDetail = async (url = '', logCallback) => {
    ...
    if (logCallback) {
        logCallback(`${url} : detect #img.src`);
    }
 ...
 * @param {(log: string) => T} logCallback - A function that will be called with the local file path of the downloaded
 * image.
 ...
 * @param {(log: string) => T} logCallback - A function that will be called with the downloaded image path.
...
const handleDownloadImage = async (imageNumber = 0, urlImage = '', downloadFolder = '', retryDownload = 5, logCallback) => {
    if (retryDownload === 0) {
        ...
        const downloadedImage = await downloadImage(urlImage, downloadFolder, String(imageNumber).padStart(3, '0'))
            .catch(async () => {
                if (logCallback) {
                    logCallback(`image_number ${imageNumber} : url ${urlImage} : Retry download (${retryDownload})`);
                }
                return await handleDownloadImage(imageNumber, urlImage, downloadFolder, retryDownload - 1, logCallback);
            });

        logCallback(`image_number ${imageNumber} : url ${urlImage} : download complete at path ${downloadedImage}`);
 ...
 * @param {(log: string) => T} logCallback - A function that will be called when the script is running.
 ...
 * @param {(log: string) => T} [logCallback] - A function that will be called with the log data.
...
const downloadEH = async (url = '', logCallback = (log) => console.log({ data: log })) => {
    ...
    if (logCallback) {
        logCallback(`url ${url} : contains page ${pageLists.length}`);
    }
    let imageNumber = 0;
    ...
    for (let index = 0; index < pageLists.length; index++) {
        ...
        if (logCallback) {
            logCallback(`url ${url} : collecting info page ${index + 1} : contain images ${responseGallery.pageGalleryDetails.length}`);
        }
